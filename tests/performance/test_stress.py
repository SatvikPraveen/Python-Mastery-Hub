...
class StressPlatformAPI:
    ...
    async def stress_submit_exercise(
    ...
    async def stress_save_progress(
...
class StressTestRunner:
    ...
    async def run_breaking_point_test(self, config: StressTestConfig) -> StressTestResult:
        ...
        while current_users <= config.max_concurrent_users:
            ...
            print(
                f"  Operations: {step_result['total_operations']}, Failure rate: {failure_rate:.2f}%"
            )
            ...
            while time.time() - session_start < duration_seconds:
                ...
                try:
                    if operation == "submit":
                        ...
                        await self.api.stress_submit_exercise(
                            user_id, f"ex_{random.randint(1, 100)}", code, "token"
                        )
                    elif operation == "content":
                        await self.api.stress_get_content(
                            f"content_{random.randint(1, 1000)}", "token"
                        )
                    elif operation == "progress":
                        await self.api.stress_save_progress(
                            user_id,
                            f"lesson_{random.randint(1, 50)}",
                            {"score": 85},
                            "token",
                        )
    ...
    def _check_resource_limits(self, config: StressTestConfig) -> bool:
        ...
        return (
            cpu_usage > config.resource_limit_cpu_percent
            or memory_usage > config.resource_limit_memory_mb
        )
...
class TestResourceExhaustion:
    ...
    async def test_connection_pool_exhaustion(self, stress_api, stress_runner):
        ...
        try:
            ...
            if result.breaking_point_users:
                assert (
                    result.breaking_point_users <= stress_api.connection_pool_size + 5
                )  # Some tolerance

            # Check for connection-related errors
            connection_errors = any(
                "connection" in mode.get("primary_error", "").lower()
...
class TestFailureCascades:
    ...
    async def test_service_cascade_failure(self, stress_api, stress_runner):
        ...
        baseline_failure_rate = (
            (baseline_result.failed_operations / baseline_result.total_operations * 100)
            if baseline_result.total_operations > 0
            else 0
        )
        ...
        try:
            ...
            circuit_breaker_errors = any(
                "circuit breaker" in mode.get("primary_error", "").lower()
        ...
        async def leaky_user_session(user_id: str, duration: int):
            ...
            try:
                ...
                for i in range(20):  # Many operations
                    await stress_api.stress_submit_exercise(
                        user_id, f"ex_{i}", "def leak(): pass", "token"
                    )
...
class TestRecoveryPatterns:
    ...
    async def test_graceful_degradation_and_recovery(self, stress_runner):
        ...
        normal_failure_rate = (
            (normal_result.failed_operations / normal_result.total_operations * 100)
            if normal_result.total_operations > 0
            else 0
        )
        ...
        print(
            f"  Under stress failure rate: {(stress_result.failed_operations / stress_result.total_operations * 100) if stress_result.total_operations > 0 else 0:.2f}%"
        )
