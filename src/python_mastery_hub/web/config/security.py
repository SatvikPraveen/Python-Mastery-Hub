...
class SecurityConfig:
    ...
    def get_password_strength_requirements(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "forbiddenPasswords": self.password_policy.forbidden_passwords[
                :10
            ],  # Send only first 10
        }
...
class SecurityAudit:
    ...
    def _audit_password_policy(self) -> tuple[int, List[Dict[str, Any]]]:
        ...
        if (
            self.config.password_policy.require_uppercase
            and self.config.password_policy.require_lowercase
        ):
            score += 10
        else:
            findings.append(
