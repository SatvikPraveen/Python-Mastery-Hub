...
print(result)""",
    ...
    def _show_solution(self, exercise: Exercise) -> None:
        ...
        confirm = input(
            f"{colors.YELLOW}Are you sure you want to see the solution? (y/n): {colors.RESET}"
        )
        if confirm.lower() != "y":
            return
    ...
    async def _get_multiline_code(self) -> str:
        ...
        print(
            f"{colors.CYAN}Enter your code (press Ctrl+D or type 'END' on a new line to finish):{colors.RESET}"
        )
...
def setup_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--module", "-m", help="Start with a specific module")
