...
locals {
  ...
  secondary_region = var.aws_region == "us-east-1" ? "us-west-2" : (
    var.aws_region == "us-west-2" ? "us-east-1" : (
    var.aws_region == "eu-west-1" ? "eu-central-1" : (
    var.aws_region == "eu-central-1" ? "eu-west-1" : "us-east-1"
  )))
...
resource "aws_s3_bucket" "app_storage_replica" {
  provider = aws.secondary
  bucket   = "${local.cluster_name}-app-storage-replica"

  tags = merge(local.common_tags, {
    Name = "${local.cluster_name}-app-storage-replica"
    Type = "ApplicationStorage"
    Role = "Replica"
  })
}

resource "aws_s3_bucket_versioning" "app_storage_replica" {
  provider = aws.secondary
  bucket   = aws_s3_bucket.app_storage_replica.id
  versioning_configuration {
    status = "Enabled"
  }
}

resource "aws_s3_bucket_encryption" "app_storage_replica" {
  provider = aws.secondary
  bucket   = aws_s3_bucket.app_storage_replica.id
  ...
  server_side_encryption_configuration {
    rule {
      apply_server_side_encryption_by_default {
        sse_algorithm = "aws:kms"
        # Use replica region key if available, otherwise use default encryption
      }
      bucket_key_enabled = true
    }
  }
...
resource "aws_s3_bucket_public_access_block" "app_storage_replica" {
  provider = aws.secondary
  bucket   = aws_s3_bucket.app_storage_replica.id
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "${aws_s3_bucket.app_storage_replica.arn}/*",
          "${aws_s3_bucket.logs_replica.arn}/*",
          "${aws_s3_bucket.backups_replica.arn}/*"
        ]
      }
    ]
  })
...
# Replication configuration for app_storage
resource "aws_s3_bucket_replication_configuration" "app_storage" {
  depends_on = [aws_s3_bucket_versioning.app_storage]
  ...
  rule {
    ...
    destination {
      bucket       = aws_s3_bucket.app_storage_replica.arn
...
resource "aws_s3_bucket" "logs_replica" {
  provider = aws.secondary
  bucket   = "${local.cluster_name}-logs-replica"

  tags = merge(local.common_tags, {
    Name = "${local.cluster_name}-logs-replica"
    Type = "Logging"
    Role = "Replica"
  })
}
...
# Backups replica bucket
resource "aws_s3_bucket" "backups_replica" {
  provider = aws.secondary
  bucket   = "${local.cluster_name}-backups-replica"

  tags = merge(local.common_tags, {
    Name = "${local.cluster_name}-backups-replica"
    Type = "Backups"
    Role = "Replica"
  })
}
...
resource "aws_s3_bucket_encryption" "backups_replica" {
  provider = aws.secondary
  bucket   = aws_s3_bucket.backups_replica.id
